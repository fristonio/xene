...
type Walker struct {
	...
	diagsMap       map[Vertex]*xerr.MultiError
...
func (w *Walker) walkVertex(v Vertex, info *walkerVertex) {
	...
	diags := xerr.NewMultiError()
	var upstreamFailed bool
	if depsSuccess {
		log.Debugf("[TRACE] dag/walk: visiting %q", VertexName(v))
		diags = w.Callback(v)
	} else {
		log.Debugf("[TRACE] dag/walk: upstream of %q errored, so skipping", VertexName(v))
		...
		diags.Append(errors.New("upstream dependencies failed"))
		upstreamFailed = true
	}
	...
	w.diagsLock.Lock()
	if w.diagsMap == nil {
		w.diagsMap = make(map[Vertex]*xerr.MultiError)
	}
	w.diagsMap[v] = diags
	...
	DepSatisfied:
		for {
			...
			case <-time.After(time.Second * 5):
				log.Debugf("[TRACE] dag/walk: vertex %q is waiting for %q",
