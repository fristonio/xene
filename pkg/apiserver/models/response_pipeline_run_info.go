// Code generated by go-swagger; DO NOT EDIT.

package models

// This file was generated by the swagger tool.
// Editing this file might prove futile when you re-run the swagger generate command

import (
	"github.com/go-openapi/strfmt"
	"github.com/go-openapi/swag"
)

// ResponsePipelineRunInfo response pipeline run info
//
// swagger:model response.PipelineRunInfo
type ResponsePipelineRunInfo struct {

	// agent
	Agent string `json:"agent,omitempty"`

	// run ID
	RunID string `json:"runID,omitempty"`

	// status
	Status string `json:"status,omitempty"`
}

// Validate validates this response pipeline run info
func (m *ResponsePipelineRunInfo) Validate(formats strfmt.Registry) error {
	return nil
}

// MarshalBinary interface implementation
func (m *ResponsePipelineRunInfo) MarshalBinary() ([]byte, error) {
	if m == nil {
		return nil, nil
	}
	return swag.WriteJSON(m)
}

// UnmarshalBinary interface implementation
func (m *ResponsePipelineRunInfo) UnmarshalBinary(b []byte) error {
	var res ResponsePipelineRunInfo
	if err := swag.ReadJSON(b, &res); err != nil {
		return err
	}
	*m = res
	return nil
}
